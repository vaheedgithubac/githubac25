module "jenkins_sg" {
  ...
  sg_name        = "jenkins_sg"
  sg_description = "Jenkins Security Group"
...
# Jenkins should be accessed from office n/w   0.0.0.0/0 ---> jenkins
resource "aws_security_group_rule" "jenkins_ssh" {
  ...
  security_group_id = module.jenkins_sg.sg_id
}

# Jenkins should be accessed from office n/w   0.0.0.0/0 ---> bastion
...
module "jenkins_ec2" {
  ...
  sg_id                       = [module.jenkins_sg.sg_id] #[local.sg_id]
  subnet_id                   = var.subnet_id
  associate_public_ip_address = true
  what_type_instance          = "jenkins"
...
resource "null_resource" "ansible_provisioner" {
  ...
  triggers = {
    instance_id = module.jenkins_ec2.ec2_instance.id
  }
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      ansible-playbook -i ${module.jenkins_ec2.ec2_instance.private_ip}, jenkins_setup.yml
    EOT
  }
}
