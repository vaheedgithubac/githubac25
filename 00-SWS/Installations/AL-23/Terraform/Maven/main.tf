module "maven_sg" {
  ...
  sg_name        = "maven_sg"
  sg_description = "Maven Security Group"
...
# Maven should be accessed from office n/w   0.0.0.0/0 ---> jenkins
resource "aws_security_group_rule" "maven_ssh" {
  type              = "ingress"
  from_port         = 22
  to_port           = 22
  protocol          = "tcp"
  cidr_blocks       = ["0.0.0.0/0"]
  security_group_id = module.maven_sg.sg_id
}
...
module "maven_ec2" {
  ...
  sg_id                       = [module.maven_sg.sg_id] #[local.sg_id]
  subnet_id                   = var.subnet_id
  associate_public_ip_address = true
  what_type_instance          = "maven"
...
resource "null_resource" "maven_provisioner" {
  depends_on = [module.maven_ec2]

  # Changes to any instance of the instance requires re-provisioning
  triggers = {
    instance_id = module.maven_ec2.ec2_instance.id
  }

  connection {
    type        = "ssh"
    user        = "ec2-user"
    host        = module.maven_ec2.ec2_instance.private_ip
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      ansible-playbook -i ${module.maven_ec2.ec2_instance.private_ip}, maven_setup.yml
    EOT
  }
}
