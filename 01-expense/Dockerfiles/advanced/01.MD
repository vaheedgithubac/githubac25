...
# Ensure config file permissions (Not necessary, image has already)
...
- USER `nginx` → already set
- listen `8080` → expected
- Correct permissions → already done
- CMD ["nginx", "-g", "daemon off;"] → already defined
...
## ✅ Hardened Kubernetes Deployment (RECOMMENDED)
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  ...
  template:
    ...
    spec:
      ...
      - name: nginx
        ...
        # 🔐 Security Context (THIS IS THE KEY PART)
        securityContext:
          runAsNonRoot: true
