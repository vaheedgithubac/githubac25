...
```
STEP 3️⃣ – Create ApplicationSet ⚙️  
...
```
👉 This creates Applications like:
```bash
expense-ui
expense-dashboard
```

## STEP 4️⃣ – Define RBAC (argocd-rbac-cm) 🛂
...
```
## STEP 5️⃣ – Restart Argo CD (IMPORTANT 🔄)
...
argocd app sync expense-ui   # ❌ should fail
```
Debug permissions  
```bash
...
## STEP 7️⃣ – What ApplicationSet Changes (Important)  
ApplicationSet:
...
```text
User → argocd-cm → Login OK
User → argocd-rbac-cm → Permissions OK
ApplicationSet → AppProject → Allowed
RBAC → AppProject/App → Visible
```
